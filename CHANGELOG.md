...
- **fix(radius):** `preserve_source_ip` теперь реально работает. Было: NAS-IP
  подставлялся только `if (!&NAS-IP-Address)` — а реальные NAS (UAG и т.п.) сами
  шлют NAS-IP, поэтому подстановка молча пропускалась («галку поставил — толку
  нет»). Стало: при галке `radiuspanel_srcip` **всегда перезаписывает**
  `NAS-IP-Address := %{Packet-Src-IP-Address}` — 2FA видит IP реального
  источника, а не прокси. Транспорт не меняется, ответ 2FA приходит нам.
  Проверено `freeradius -X` (пришло NAS-IP=9.9.9.9 → на home ушло packet-src).
...
- **fix(pool):** `username_uppercase` не возвращался в ответе API — `pools.py::_serialize`
  собирал `PoolOut` вручную и не передавал новое поле (crud писал в БД верно). UI
  из-за этого показывал снятую галку после Save. Добавлено в `_serialize`.
...
- **docs:** README + инструкция (Help) обновлены под ADR-0010/0011: разделы
  health-check таргета, аварийный обход 2FA, уведомления по почте; таблица
  разделов UI (Mail, Guide, health-check у Target servers).
- **fix(ui-i18n):** весь UI приведён к английскому (интерфейс на английском по
  соглашению проекта); русской осталась только сама инструкция (Help.jsx).
  Вычищен русский, занесённый в предыдущих правках: поля/хинты таргета
  (health-check), текст правила fallback, RADIUS-хинт, Mail-вкладка; нав-пункт
  `Инструкция` → `Guide`; вкладка `Почта` → `Mail`.
- **feat(ADR-0011):** SMTP-настройки в Settings → вкладка «Mail» (host/port/
